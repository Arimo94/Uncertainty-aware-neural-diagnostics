...
def residual_0(z,state,params,Ts):
    """ RESIDUAL_0 Sequential residual generator for model 'LiU_ICE model'
    Causality: int

    Structurally sensitive to faults: fyw_af, fyp_im
    ...
    state = {'wg_pos': wg_pos_0, 'omega_tc': omega_tc_0, 'T_t': T_t_0, 'm_t': m_t_0, 'T_em': T_em_0, 'm_em': m_em_0, 'T_im': T_im_0, 'm_af': m_af_0, 'T_af': T_af_0, 'm_c': m_c_0, 'T_c': T_c_0, 'm_ic': m_ic_0, 'T_ic': T_ic_0}
    for k,zk in enumerate(z):
        r[k], state = residual_0( zk, state, params, Ts )

    State is a dictionary with the keys: wg_pos, omega_tc, T_t, m_t, T_em, m_em, T_im, m_af, T_af, m_c, T_c, m_ic, T_ic

    File generated Wed Feb 26 12:02:26 2025
    """
    def ApproxInt(dx, x0, Ts):
        return x0 + Ts*dx

    def residual_0_core(z, state, params, Ts):
        # Parameters
    ...
    return residual_0_core(z, state, params, Ts)
